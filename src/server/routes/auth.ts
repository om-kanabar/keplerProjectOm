...
export function registerAuthRoutes(app: Hono): void {
  app.post("/auth/web", (c) => {
    ...
    const expiresAt = new Date(Date.now() + WEB_CODE_TTL_MS);
    webLoginCodes.set(hashSecret(code), { expiresAt });
    removeExpiredCodes();
...
function removeExpiredCodes(): void {
  const now = Date.now();

  for (const [code, login] of webLoginCodes) {
    if (login.expiresAt.getTime() <= now) {
