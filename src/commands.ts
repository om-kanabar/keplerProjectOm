...
export async function runCli(argv: string[]): Promise<void> {
  ...
  const clockCommand = program.command("clock").description("Control the local Kepler live clock.");
  clockCommand.command("status").action(async () => { try { const result = await apiClient.getClockStatus(); respond(result, () => console.log(JSON.stringify(result.clock, null, 2))); } catch (error) { fail(error instanceof Error ? error.message : "Unable to read clock status."); } });
  clockCommand.command("listen").argument("<mode>", "on or off").action(async (mode: string) => { try { if (mode !== "on" && mode !== "off") throw new Error("Clock listening mode must be `on` or `off`."); const result = await apiClient.setClockListening(mode === "on"); respond(result, () => console.log(`Kepler listening ${mode}.`)); } catch (error) { fail(error instanceof Error ? error.message : "Unable to change clock listening."); } });
  clockCommand.command("watch").action(async () => { const controller = new AbortController(); process.once("SIGINT", () => controller.abort()); try { await apiClient.watchClock((event) => console.log(`planet_tick tick=${event.absoluteTick} advancedBy=${event.advancedBy} issuedAt=${event.issuedAt} applied=${event.applied}`), controller.signal); } catch (error) { if (!controller.signal.aborted) fail(error instanceof Error ? error.message : "Unable to watch clock events."); } });
