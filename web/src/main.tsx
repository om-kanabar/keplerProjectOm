...
function Dashboard() {
  ...
  return <main className={`habitat-dashboard console-layout ${activeMode === "display" ? "display-mode" : ""} ${displayChromeVisible ? "display-chrome-visible" : "display-chrome-hidden"}`}><ClockPanel clock={snapshot.clock} />
...
function ClockPanel({ clock: rawClock }: { clock?: RegularModeSnapshot["clock"] }) { const clock = rawClock ?? { mode: "manual" as const, listening: false, connectionStatus: "disconnected", latestKeplerTick: null, latestAdvancedBy: null, lastError: null }; const change = async (enabled: boolean) => { await fetch("/clock/listen", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ enabled }) }); window.location.reload(); }; return <section className="overview-section clock-panel" aria-label="Live clock"><SectionHeading id="clock-heading" label="Live clock" detail={`${clock.mode} mode · ${clock.connectionStatus}`} /><div className="clock-metrics"><span>Listening <strong>{clock.listening ? "on" : "off"}</strong></span><span>Latest tick <strong>{clock.latestKeplerTick ?? "—"}</strong></span><span>Advanced by <strong>{clock.latestAdvancedBy ?? "—"}</strong></span></div><div className="clock-actions"><button className="button" disabled={clock.listening} onClick={() => void change(true)}>Listen on</button><button className="button" disabled={!clock.listening} onClick={() => void change(false)}>Listen off</button></div>{clock.lastError && <p className="dashboard-feedback error">{clock.lastError}</p>}</section>; }
