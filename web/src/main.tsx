...
const subsystems = ["Overview", "Modules", "Blueprints", "Resources", "Inventory", "Construction", "Alerts", "Forecast", "Humans", "EVA", "Scan", "Catalogs", "World", "Server", "Settings"];
...
function DisplayModeView({ snapshot, onExit, error }: { snapshot: RegularModeSnapshot; onExit: () => void; error: string | null }) {
  ...
  return <section className="display-mode-view" aria-label="Ambient habitat display"><span className="display-mode-exit" role="button" tabIndex={0} aria-label="Exit display mode" title="Exit display mode" onClick={onExit} onKeyDown={exit}>←</span><div className="display-mode-topline"><span>{snapshot.registration?.displayName ?? "Habitat"}</span><time>{clock}</time></div><div className="display-mode-hero"><p className="dashboard-label">{snapshot.connection === "connected" ? "Habitat status" : "Connection degraded"}</p><h2 className="display-status-line">{snapshot.overall.label}</h2><p>{pickOperatingLine()}</p></div><div className="display-mode-metrics"><DisplayMetric label="Power" value={byId("power")?.value ?? "Unavailable"} detail={byId("power")?.interpretation ?? "Awaiting telemetry"} /><DisplayMetric label="Battery" value={byId("battery")?.value ?? "Unavailable"} detail={byId("battery")?.interpretation ?? "Awaiting telemetry"} /><DisplayMetric label="Solar" value={byId("solar")?.value ?? "Unavailable"} detail={byId("solar")?.interpretation ?? "Awaiting telemetry"} /><DisplayMetric label="Reserve" value={byId("reserve")?.value ?? "Unavailable"} detail={byId("reserve")?.interpretation ?? "Awaiting telemetry"} /></div><div className="display-mode-lower"><div><p className="dashboard-label">Active work</p><strong>{snapshot.activeWork.length ? snapshot.activeWork[0].label : "No active construction"}</strong><p>{snapshot.activeWork.length ? snapshot.activeWork[0].detail : "Habitat operations are waiting."}</p></div><div><p className="dashboard-label">Alerts</p><strong>{snapshot.alerts.length ? `${snapshot.alerts.length} open` : "All clear"}</strong><p>{snapshot.alerts.length ? "Review the Alerts system when convenient." : "No conditions require attention."}</p></div></div>{error && <p className="display-mode-error" role="status">Live refresh unavailable · showing last known state</p>}</section>;
}
...
function SubsystemView({ name, snapshot, blueprints, catalogResources, inventory, onStatusChange }: { name: Subsystem; snapshot: RegularModeSnapshot; blueprints: Blueprint[] | null; catalogResources: CatalogResource[] | null; inventory: InventoryItem[] | null; onStatusChange: (moduleId: string, status: "online" | "offline" | "active") => void }) {
  ...
  if (name === "Humans") return <HumansView snapshot={snapshot} />;
  if (name === "Scan") return <ScanView />;
...
function ActionForm({ label, path, fields, onDone }: { label: string; path: string; fields: Array<{ name: string; label: string; type?: string; defaultValue?: string }>; onDone?: (message: string) => void }) {
  const [values, setValues] = useState<Record<string, string>>({});
  const [state, setState] = useState<string | null>(null);
  const submit = async (event: FormEvent) => { event.preventDefault(); setState("Working"); try { await request(path, { method: "POST", body: JSON.stringify(Object.fromEntries(Object.entries(values).map(([key, value]) => [key, /^-?\\d+(\\.\\d+)?$/.test(value) ? Number(value) : value]))) }); const message = `${label} complete.`; setState(message); onDone?.(message); } catch (caught) { setState(caught instanceof Error ? caught.message : `${label} failed.`); } };
  return <form className="command-form" onSubmit={(event) => void submit(event)}><p className="dashboard-label">{label}</p>{fields.map((field) => <label key={field.name}>{field.label}<input required name={field.name} type={field.type ?? "text"} defaultValue={field.defaultValue} onChange={(event) => setValues((current) => ({ ...current, [field.name]: event.target.value }))} /></label>)}<button className="button button-primary" type="submit">Run</button>{state && <p className="dashboard-feedback" role="status">{state}{state === "Working" && <> <LoadingDots /></>}</p>}</form>;
}

function HumansView({ snapshot }: { snapshot: RegularModeSnapshot }) { const [humans, setHumans] = useState<any[] | null>(null); useEffect(() => { void request<{ humans: any[] }>("/humans").then((result) => setHumans(result.humans)).catch(() => setHumans([])); }, []); return <section className="subsystem-view"><SectionHeading id="humans-view-heading" label="Humans" detail="Crew overview" />{humans === null ? <LoadingState label="Loading crew" /> : humans.length ? <div className="table-frame"><table className="dashboard-table"><thead><tr><th>Name</th><th>Location</th><th>Move</th></tr></thead><tbody>{humans.map((human) => <tr key={human.id}><td>{human.displayName ?? human.name ?? human.id}</td><td>{human.moduleId ?? human.location ?? "unassigned"}</td><td><ActionForm label="Move" path={`/humans/${encodeURIComponent(human.id)}/move`} fields={[{ name: "moduleId", label: "Module ID" }]} /></td></tr>)}</tbody></table></div> : <EmptyState text="No humans found." />}</section>; }
function ScanView() { return <section className="subsystem-view"><SectionHeading id="scan-view-heading" label="Scan" detail="Explorer interface" /><ActionForm label="Run surface scan" path="/scan" fields={[{ name: "sensorStrength", label: "Sensor strength", type: "number", defaultValue: "60" }, { name: "radiusTiles", label: "Radius", type: "number", defaultValue: "0" }]} /><ActionForm label="Collect resource" path="/collect" fields={[{ name: "quantityKg", label: "Quantity (kg)", type: "number" }]} /></section>; }
...
function EVAView() { const [eva, setEva] = useState<any>(null); const [humans, setHumans] = useState<any[]>([]); useEffect(() => { void Promise.all([request<{ eva: any }>("/eva"), request<{ humans: any[] }>("/humans")]).then(([e, h]) => { setEva(e.eva); setHumans(h.humans); }).catch(() => setEva({ error: "Unable to load EVA state." })); }, []); return <section className="subsystem-view"><SectionHeading id="eva-view-heading" label="EVA" detail="Explorer operations" />{eva ? <><div className="mode-placeholder"><p className="dashboard-label">Current state</p><h2>{eva.error ?? (eva.humanId ? `Human ${eva.humanId} at (${eva.x}, ${eva.y})` : "Explorer docked")}</h2></div><div className="command-grid"><ActionForm label="Deploy explorer" path="/eva/deploy" fields={[{ name: "humanId", label: "Human", defaultValue: humans[0]?.id ?? "" }]} /><ActionForm label="Move explorer" path="/eva/move" fields={[{ name: "x", label: "X", type: "number" }, { name: "y", label: "Y", type: "number" }]} /><ActionForm label="Dock explorer" path="/eva/dock" fields={[]} /></div></> : <LoadingState label="Loading EVA state" />}</section>; }
