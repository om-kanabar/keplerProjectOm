...
function ScanResult({ result }: { result: any }) { const scan = result?.scan ?? result ?? {}; const tiles = Array.isArray(scan.tiles) ? scan.tiles : []; return <div className="scan-result-cards"><p className="dashboard-label">Scan results</p>{tiles.length ? tiles.map((tile: any, index: number) => { const top = tile.topCandidate && typeof tile.topCandidate === "object" ? tile.topCandidate : {}; const candidate = top.resource ?? top.resourceId ?? top.resourceType ?? top.candidate ?? tile.topCandidate ?? "Unknown"; const estimate = tile.quantityEstimate ?? {}; const probabilities = Array.isArray(tile.probabilities) ? tile.probabilities : []; return <article key={`${tile.x}:${tile.y}:${index}`} className="scan-result-card"><div><strong>Tile ({tile.x}, {tile.y})</strong><span>{tile.terrain ?? "Unknown terrain"}</span></div><p>Likely resource <b>{candidate}</b></p>{probabilities.length > 0 && <p>Confidence {String(probabilities[0].probability ?? probabilities[0].probabilityPct ?? probabilities[0].confidence ?? "—")}%</p>}<p>Estimated amount {estimate.estimatedKilograms ?? estimate.estimatedKg ?? estimate.kilograms ?? "—"} kg</p></article>; }) : <p>No tiles were returned by the scan.</p>}</div>; }
