...
function ScanResult({ result }: { result: any }) { const scan = result?.scan ?? result ?? {}; const tiles = Array.isArray(scan.tiles) ? scan.tiles : []; useEffect(() => { window.dispatchEvent(new CustomEvent("habitat:scan", { detail: scan })); }, [scan]); const resources = tiles.map((tile: any) => scanDisplayValue(tile.topCandidate?.resourceType ?? tile.topCandidate?.resource ?? tile.topCandidate)).filter((value) => value !== "—"); return <div className="scan-result-cards"><p className="dashboard-label">Scan mapped</p><p>{tiles.length ? `${tiles.length} tiles colored on the EVA map.` : "No tiles were returned by the scan."}</p>{resources.length > 0 && <p>Leading readings: <b>{Array.from(new Set(resources)).slice(0, 3).join(", ")}</b></p>}</div>; }
function EVAScanColorizer() { useEffect(() => { const paint = (event: Event) => { const scan = (event as CustomEvent).detail ?? {}; const tiles = Array.isArray(scan.tiles) ? scan.tiles : []; const map = document.querySelector(".eva-map"); if (!map) return; tiles.forEach((tile: any) => { const button = Array.from(map.querySelectorAll<HTMLElement>(".eva-tile")).find((candidate) => candidate.getAttribute("aria-label") === `Move to ${tile.x}, ${tile.y}`); if (!button) return; const raw = tile.topCandidate?.probabilityPct ?? tile.topCandidate?.probability ?? tile.probabilityPct ?? tile.confidence ?? 0; const confidence = Math.max(0, Math.min(100, Number(raw) || 0)); button.classList.add("is-scanned"); button.style.setProperty("--scan-confidence", `${confidence / 100}`); button.title = `${tile.terrain ?? "Terrain"} · ${scanDisplayValue(tile.topCandidate?.resourceType ?? tile.topCandidate?.resource ?? tile.topCandidate)}`; }); }; window.addEventListener("habitat:scan", paint); return () => window.removeEventListener("habitat:scan", paint); }, []); return null; }
