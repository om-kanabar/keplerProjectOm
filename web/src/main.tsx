...
function ScanResult({ result }: { result: any }) { const scan = result?.scan ?? result ?? {}; const tiles = Array.isArray(scan.tiles) ? scan.tiles : []; return <div className="scan-result-cards"><p className="dashboard-label">Scan results</p>{tiles.length ? tiles.map((tile: any, index: number) => { const top = tile.topCandidate && typeof tile.topCandidate === "object" ? tile.topCandidate : {}; const candidate = scanDisplayValue(top.resource ?? top.resourceId ?? top.resourceType ?? top.candidate ?? tile.topCandidate ?? "Unknown"); const estimate = tile.quantityEstimate ?? {}; const probabilities = Array.isArray(tile.probabilities) ? tile.probabilities : []; const confidence = probabilities.length > 0 ? scanDisplayValue(probabilities[0].probability ?? probabilities[0].probabilityPct ?? probabilities[0].confidence) : "—"; return <article key={`${tile.x}:${tile.y}:${index}`} className="scan-result-card"><div><strong>Tile ({scanDisplayValue(tile.x)}, {scanDisplayValue(tile.y)})</strong><span>{scanDisplayValue(tile.terrain ?? "Unknown terrain")}</span></div><p>Likely resource <b>{candidate}</b></p><p>Confidence {confidence}%</p><p>Estimated amount {scanDisplayValue(estimate.estimatedKilograms ?? estimate.estimatedKg ?? estimate.kilograms)} kg</p></article>; }) : <p>No tiles were returned by the scan.</p>}</div>; }
